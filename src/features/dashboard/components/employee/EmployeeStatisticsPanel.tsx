...
function getReferenceDate(visits: IVisit[], restaurantId: string) {
  ...
  const visitTimes = visits
    .filter((visit) => normalizeId(visit.restaurant_id) === selectedRestaurantId)
...
function filterReviews(
  ...
  return reviews.filter((review) => {
    if (normalizeId(review.restaurant_id) !== selectedRestaurantId) return false;
...
function getTopDish(dishes: Dish[]) {
  return dishes
    .filter((dish) => dish.active !== false && (dish.avgRating || dish.ratingsCount))
    .sort(
      (a, b) =>
...
function MetricCard({ title, value, description, icon: Icon, accent }: MetricCardProps) {
  return (
...
export default function EmployeeStatisticsPanel({
  ...
  const metrics = useMemo(() => {
    ...
    const uniqueCustomerIds = new Set(
      filteredVisits.map((visit) => normalizeId(visit.customer_id)).filter(Boolean),
    );
    const customerVisitCounts = filteredVisits.reduce<Record<string, number>>((acc, visit) => {
    ...
    const loyaltyRate =
      uniqueCustomerIds.size > 0 ? (recurrentCustomers / uniqueCustomerIds.size) * 100 : 0;
    const averageTicket = totalVisits > 0 ? totalRevenue / totalVisits : 0;
    const averageRating =
      filteredReviews.length > 0
        ? filteredReviews.reduce((sum, review) => sum + Number(review.globalRating ?? 0), 0) /
      ...
      <div className="mx-auto flex w-full max-w-7xl flex-col gap-8">
        <header className="flex flex-col justify-between gap-5 rounded-[2rem] border border-slate-100 bg-white p-7 shadow-sm md:flex-row md:items-center">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.24em] text-orange-500">
              {t('dashboard.employee.statisticsLabel', 'Rendimiento del negocio')}
            </p>
            <h1 className="mt-2 text-3xl font-black tracking-tight text-slate-900">
              {t('dashboard.employee.statisticsTitle', 'Panel de estadísticas del restaurante')}
            </h1>
        ...
        <section className="grid grid-cols-1 gap-6 lg:grid-cols-2">
          <article className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-sm flex min-h-[390px] flex-col">
            <div className="mb-5 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  {t('components.revenueLine.subtitle', 'Tendencia económica del restaurante')}
                </p>
              </div>
            ...
            <div className="mt-4 h-[300px] overflow-hidden">
              <div className="mt-4 h-[300px] overflow-visible">
                <RevenueLineChart visits={filteredVisits} restaurantId={restaurantId} />
              </div>
            </div>
          </article>
          ...
          <article className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-sm flex min-h-[390px] flex-col">
            <div className="mb-5 flex items-center justify-between">
              <div>
                <h2 className="text-sm font-black uppercase tracking-[0.18em] text-slate-500">
                  {t('components.customerDonut.title', 'Clientes nuevos y recurrentes')}
                </h2>
                <p className="mt-1 text-sm text-slate-400">
                  {t('components.customerDonut.subtitle', 'Distribución de fidelización')}
                </p>
              </div>
            ...
            <div className="mt-4 h-[300px] overflow-hidden">
              <div className="mt-4 h-[300px] overflow-visible">
                <CustomerDonutChart visits={filteredVisits} restaurantId={restaurantId} />
              </div>
            </div>
          </article>
          ...
          <article className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-sm flex min-h-[390px] flex-col">
            <div className="mb-5 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  {t('components.topDishes.subtitle', 'Platos con mejor rendimiento')}
                </p>
              </div>
          ...
          <article className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-sm flex min-h-[390px] flex-col">
            <div className="mb-5 flex items-center justify-between">
              <div>
                ...
                <p className="mt-1 text-sm text-slate-400">
                  {t('components.peakHours.subtitle', 'Franjas horarias con más actividad')}
                </p>
              </div>
            ...
            <div className="mt-4 h-[300px] overflow-hidden">
              <div className="mt-4 h-[300px] overflow-visible">
                <PeakVisitHoursChart visits={filteredVisits} restaurantId={restaurantId} />
              </div>
            </div>
          </article>
        </section>
        ...
        <section className="grid grid-cols-1 gap-5 md:grid-cols-3">
          ...
          <div className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-3xl font-black text-slate-900">{bestHour?.[0] ?? '--:--'}</p>
            <p className="mt-2 text-sm text-slate-500">
              {bestHour
                ? `${bestHour[1]} ${t('dashboard.employee.bestHourVisits', 'visitas registradas')}`
                : t('dashboard.employee.noBestHour', 'Todavía no hay datos suficientes.')}
            </p>
          </div>
          ...
          <div className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-sm">
            ...
            <p className="mt-2 text-2xl font-black text-slate-900">{topDish?.name ?? '-'}</p>
            <p className="mt-2 text-sm text-slate-500">
              {topDish
                ? `${Number(topDish.avgRating ?? 0).toFixed(1)} ★ · ${Number(topDish.ratingsCount ?? 0)} ${t('dashboard.employee.ratings', 'valoraciones')}`
                : t('dashboard.employee.noBestDish', 'No hay platos con datos suficientes.')}
            </p>
          </div>
        </section>
      </div>
