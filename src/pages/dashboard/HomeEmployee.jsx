...
import VisitsPerHourChart from '../../components/dashboard/VisitsPerHourChart';
import RatingsBarChart from '../../components/dashboard/RatingsBarChart';
import {
  getRestaurantById,
  getRecentVisits,
  getRestaurantKpis,
...
export default function HomeEmployee() {
  ...
  const [visitsPerHour, setVisitsPerHour] = useState([]);
  ...
  useEffect(() => {
    async function fetchData() {
      if (!user?.restaurant_id) {
        setApiError('Tu usuario no tiene restaurante asignado.');
        setLoading(false);
        return;
      }

      try {
        const [restaurantData, recentVisitsData, kpisData, visitsPerHourData, ratingsData] =
          await Promise.all([
            getRestaurantById(user.restaurant_id, token),
            getRecentVisits(user.restaurant_id, token, 5),
            getRestaurantKpis(user.restaurant_id, token),
            getVisitsPerHour(token, { restaurantId: user.restaurant_id, startHour: 10, endHour: 23 }),
            getAverageRatingsByRestaurant(user.restaurant_id, token),
          ]);

        setRestaurant(restaurantData ?? null);
        setVisits(recentVisitsData ?? []);
        setKpis(kpisData);
        setVisitsPerHour(visitsPerHourData);
        setRatings(ratingsData ?? []);
        setApiError('');
      } catch (err) {
        setApiError(err?.message ?? 'No se pudieron cargar las estadísticas del dashboard.');
          ...
          <div className="dash-header__left">
            <div className="brand brand--sm">
              ...
              <span className="brand-name">
                EasyEat <span className="badge badge--brand">{role.toUpperCase()}</span>
              </span>
            </div>
          </div>
        ...
        <div className="employee-layout">
          <div className="employee-main">
            <section className="welcome-banner glass-card">
              <div className="welcome-banner__text">
                ...
                <p>
                  Gestionando el restaurante:{' '}
                  <strong>{restaurant?.profile?.name || 'Cargando...'}</strong>
                </p>
                {restaurant?.profile?.location?.address && (
                  <p className="res-loc">📍 {restaurant.profile.location.address}</p>
                )}
              </div>
            </section>

            <section className="section">
              <h2 className="section-title">Resumen del restaurante</h2>
              {apiError && (
            ...
            <section className="section">
              ...
              <div className="action-grid">
                <button className="action-card glass-card">
                  <div className="action-icon action-icon--plus">
                    <PlusCircle />
                  </div>
                ...
                <button className="action-card glass-card">
                  <div className="action-icon action-icon--award">
                    <Award />
                  </div>
                ...
                <button className="action-card glass-card">
                  <div className="action-icon action-icon--qr">
                    <QrCode />
                  </div>
            ...
            <section className="section">
              ...
              {loading ? (
                ...
                <div className="employee-charts-grid">
                  <VisitsPerHourChart data={visitsPerHour} />
                  <RatingsBarChart data={ratings} />
                </div>
              )}
            </section>
            ...
            <section className="section recent-activity">
              <div className="section-header">
                <h2 className="section-title">
                  <Clock size={20} /> Visitas recientes
                </h2>
          ...
          <aside className="employee-sidebar">
            <div className="stats-card glass-card">
              ...
              <p className="stat-trend trend-up">Últimos movimientos</p>
            </div>
